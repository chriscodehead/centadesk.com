...
if (isset($_GET['status'])) {
 ...
 if ($_GET['status'] == 'cancelled') {
  // echo 'YOu cancel the payment';
    ...
    $email = $row['email'];
    $subject = $siteName." - Cancelled Payment.";
    $message = 'Phone: '.$row['phone'].', Email: '.$row['email'];
    $email_call->contactUsMail($fullname, $email, $subject, $message);
    query_sql("DELETE FROM $user_tb WHERE `user_code`='".$txRef."' LIMIT 1");
    header('Location: apply?cancel=cancelled');
