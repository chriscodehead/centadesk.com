...
if (isset($_POST['post'])) {
  ...
  $cat_id = $bassic->randGenerator();
  $cover = '';

  if (!empty($title)) {
    if (query_sql("INSERT INTO $course_cat_tb VALUES(null, '" . $theurl . "', '" . $title . "', '" . $cover . "' ) ")) {
...
if (isset($_GET['iddel']) && !empty($_GET['iddel'])) {
  if (query_sql("DELETE FROM $course_cat_tb WHERE `id`='" . $_GET['iddel'] . "' LIMIT 1")) {
