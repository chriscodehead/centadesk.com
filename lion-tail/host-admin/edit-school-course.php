...
if (isset($_POST['post'])) {
 ...
 if (!empty($covercode) && !empty($url) && !empty($title) && !empty($course_cat)) {
  ...
  $fieldup = array("course_cat", "title", "url", "cover", "date_add");
  $valueup = array($course_cat, $title, $url, $covercode, $bassic->getDate2());
  $update = $cal->update($school_course_tb, $fieldup, $valueup, 'id', $_GET['id']);
  if (!empty($update)) {
   $msg = $update;
  } else {/*do nothing*/
  }
    ...
    <div class="row">
     <div class="col-lg-12">
      ...
      <ol class="breadcrumb">
       ...
       <li><i class="fa fa-laptop"></i><a href="create-school-course"> Create School Courses</a></li>
      </ol>
     </div>
    </div>
        ...
        <tbody>
         <tr>
          ...
          <td> <select class="form-control" name="course_cat">
            ...
            <?php $cat = $cal->selectFrmDB($school_course_tb, 'course_cat', 'id', $_GET['id']); ?>
         ...
         <tr>
          <td>Course Title</td>
          <td><input class="form-control" value="<?php print $cal->selectFrmDB($school_course_tb, 'title', 'id', $_GET['id']); ?>" name="title">
          </td>
         </tr>

         <tr>
          <td> Cover Code</td>
          <td> <input name="covercode" value="<?php print $cal->selectFrmDB($school_course_tb, 'cover', 'id', $_GET['id']); ?>" type="text" class="form-control" id="" /></td>
         </tr>
         <tr>
          <td> Page Link</td>
          <td> <input name="url" value="<?php print $cal->selectFrmDB($school_course_tb, 'url', 'id', $_GET['id']); ?>" type="text" class="form-control" id="" /></td>
         </tr>
         <tr>
          <td></td>
